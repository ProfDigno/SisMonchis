...
public class EvenJTextField {
    ...
    public int getInt_sumar_restar_cantidad(java.awt.event.KeyEvent evt,JTextField txtcantidad,boolean venta_negativo,double stock_actual) {
        int Icant_actual = 0;
        if (evt.getKeyCode() == KeyEvent.VK_DOWN) {
            if (txtcantidad.getText().trim().length() > 0) {
                String cant_actual = txtcantidad.getText().trim();
                Icant_actual = Integer.parseInt(cant_actual);
                Icant_actual = Icant_actual - 1;
                if (Icant_actual <= 0) {
                    Icant_actual = 1;
                }
                txtcantidad.setBackground(Color.white);
                txtcantidad.setText(String.valueOf(Icant_actual));
            }
        }
        if (evt.getKeyCode() == KeyEvent.VK_UP) {
            if (venta_negativo) {
                if (txtcantidad.getText().trim().length() > 0) {
                    String cant_actual = txtcantidad.getText().trim();
                    Icant_actual = Integer.parseInt(cant_actual);
                    int Istock_actual = (int) stock_actual;
