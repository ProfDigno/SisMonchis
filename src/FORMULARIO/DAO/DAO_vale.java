...
public class DAO_vale {
    ...
    private String sql_insert = "INSERT INTO public.vale(\n"
            + "            idvale, fecha_emision, descripcion, monto_vale,estado, fk_idcliente, \n"
            + "            fk_idusuario)\n"
            + "    VALUES (?, ?, ?, ?, ?, ?, ?);";
    ...
    private String sql_anular = "UPDATE public.vale\n"
            + "   SET estado=?\n"
            + " WHERE idvale=?;";
    private String sql_select = "select v.idvale,('indi') as indi,to_char(v.fecha_emision,'yyyy-MM-dd HH24:MI') as fecha,\n"
            + "v.descripcion,cl.nombre as cliente,\n"
            + "TRIM(to_char(v.monto_vale,'999G999G999')) AS monto,\n"
            + "v.estado,('pc') as equipo\n"
